function    movedot(window,dotcenter,dotcenter2,foreground_lum,foreground_lum2,background_lum,angle,angle2,dotsize,dotsize2,pre,amp_mod_vis,Freq_mod,duration,vis_offset,stim_curve,distance,distance2,play_vis, size_change);
...
%     DJT 2/6/2014
    ...
    scr_num=length(vis_pos_x); %= how many times the screen will update during a stimulus
...
if play_vis(2)==1 %% go ahead and calculate the vis positions (in pixels) for vis2
    ...
    if size_change
        dotsize2=[dotsize2-size_change/2+size_step/2:size_step:dotsize2+size_change/2-size_step/2];
    end
...
%         vis_pix_min
%         vis_pix_max
%         Screen(window,'FillOval',foreground_lum,[vis_pix_min(1,1),vis_pix_max(2,1),vis_pix_max(1,1),vis_pix_min(2,1)]);
        Screen(window,'FillOval',foreground_lum,[vis_pix_min(1,1),vis_pix_min(2,1),vis_pix_max(1,1),vis_pix_max(2,1)]);
        for i=[2:scr_num];
        Screen(window,'WaitBlanking');
%         Screen(window,'FillOval',background_lum,[vis_pix_min(1,i-1),vis_pix_max(2,i-1),vis_pix_max(1,i-1),vis_pix_min(2,i-1)]);
...
%         Screen(window,'FillOval',foreground_lum,[vis_pix_min(1,i),vis_pix_max(2,i),vis_pix_max(1,i),vis_pix_min(2,i)]);
        end;
        Screen(window,'WaitBlanking');
...
% Using priority instead of Rush DJT 2/4/2014
...
%%MAKE THE SCREENS  %%% Moved 2/4/2014
% if play_vis(1)==0&&play_vis(2)==0   %%no vis, aka background==foreground
%     prescreens=round(((snd.hz/1000)*(pre+snd.post)))-1;
%     
%     loop={                
...
%%% Commented out 2/4/2013 DJT ... changing from 'Rush' to 'Priority'
% if play_vis(1)==0&&play_vis(2)==1
%     loop={
%         'Screen(window,''WaitBlanking'');'
%         'Screen(window,''fillRect'',background_lum);'
