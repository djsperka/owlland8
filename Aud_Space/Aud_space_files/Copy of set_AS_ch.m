...
% subplot (1,2,1)
            ha1=axes('units','character','position', [50 10 50 14],'tag','waveform','nextplot','add'); %Make the window
...
%             keyboard
            while rec.dispwave==1
                ...
                while iii<plotpoints %For for every point along X to occur
                    iii=invoke(RA_16,'gettagval','Oscindex');
                    
                    drawnow;
                end
                
                iiii=double(invoke(RA_16,'gettagval','OscWaveIndex')); 
                %while iiii<plotpoints %For for every point along X to occur
                %    iiii=invoke(RA_16,'gettagval','OscWaveIndex');
                %    
                %    drawnow;
                %end
                ...
                if iiii >= 32
                    threshwave=invoke(RA_16,'readtagv','OscWave',0,iiii); %Then grab the full wave
                end
                temp_min=min(fullwave);
                temp_max=max(fullwave);
                %temp_mint=min(threshwave);
                %temp_maxt=max(threshwave);
                ...
                %if temp_mint<threshwave_min
                %    threshwave_min=temp_min;
                %end
                %if temp_maxt>threshwave_max
                %    threshwave_max=temp_max;
                %end
                switch rec.OscCh
                    case 1
...
%                 ha2=axes('units','character','position', [50 10 100 14],'tag','waveform','nextplot','add'); %Make the window
...
%                     set(lh1,'xdata',[1:length(fullwave)],'ydata',fullwave);  %draws the continuous waveform
                    ...
                    %axes(ha1);  %make axes for fullwave active, somehow
                    %removes ability to interact with gui
                    ...
                    if iiii >= 32
                        plot(plotpoints+100:plotpoints+99+iiii,threshwave,'b');
                        set(gca,'nextplot','add') %not sure what this is doing
                    end
