function collect_spike_training_AS()
    ...
    compression_SF=4681000;
        % scale factor used for compressing data in RPvds file. This is just an estimate to get the
        % signal into the right range to utilize all the precision; will divide by this same number
        % again later.  Arrived at this estimate using this logic: amplifier range = 7mV 
        % or 0.007V, and 32767 is TDT SF for 1 V signal.  compression_SF =
        % 32767/.007 = 4681000. 
    ...
    % set SF
    invoke(RA_16, 'SetTagVal','ScaleFactor',compression_SF/rec.gain);  
        % multiply signal from preamp by compression_SF; also divide signal by
        % rec.gain to undo the earlier multiplication by rec.gain
    ...
    for j=1:length(channels)
        ...
        wave = invoke(RA_16,'ReadTagVEX',strcat('Ch',num2str(channels(j)),'wave'),1,buffer_size,'I16','I16',1); % *don't* uncompress data
        rawwave{channels(j)} = wave; %/compression_SF;  % *can't divide with int16.* this converts the waveform back into the actual voltage from the brain (undoes all scaling)
