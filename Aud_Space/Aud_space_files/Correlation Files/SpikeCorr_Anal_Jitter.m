...
function [mean_matrix,sem_matrix] =SpikeCorr_Anal_Jitter (spikes, chan)
global fs maxlag
name='Jitt';
...
XMatrix=zeros((trial_total*spikes.reps*jitt_iterations),bin_total,pair_total); 
...
for jitt_count=1:jitt_iterations 
    ...
    spike_jitter.datatime=time_jitter;
...
for r = 1:K
    for c = r:K  % upper right triangle only (b/c symmetric)
        ...
        trialcounter=1; %Needs to be inside column and row loops
        for reptag=1:spikes.reps;
            for trialtag=1:trial_total;
                ...
                selectcol = find(spike_jitter.datachan == chan(c)& spike_jitter.datatrial==trialtag & spike_jitter.datarep==reptag);
                
                if ((length(selectrow) > 1) && (length(selectcol) > 1)) %If there are spikes in both channels
                    %Calculate the correlation
                    %             keyboard %%GOOD debugging point
                    [cross,lags] = pxcorr(spikes.datatime(selectrow), spike_jitter.datatime(selectcol), fs, maxlag);
                    ...
                    XMatrix(trialcounter+trial_total*spikes.reps*(jitt_count-1),:,paircounter)=cross/spikeproduct;
...
mean_matrix=trial_collapse/(trial_total*spikes.reps*jitt_iterations); 
