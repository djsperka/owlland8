...
function [shuffle_cell]=SpikeCorr_Anal_Shuffle (spikes, chan)
global fs maxlag 
...
%Make 2 sets of data outputs; one for efficacy, one for contribution
...
XMatrix1=zeros((trial_total*spikes.reps*shuff_iterations),bin_total,pair_total); 
XMatrix2=zeros((trial_total*spikes.reps*shuff_iterations),bin_total,pair_total); 
...
for shuff_count=1:(shuff_iterations)
   tic 
    ...
    %%%%%DIAGNOSTICS IN CODE DUMP AT BOTTOM. INSERT HERE AND COMMENT IN TO
    %%%%%ACTIVATE
...
%                 keyboard
                if ((length(selectrow) > 1) && (length(selectcol) > 1)) %If there are spikes in both channels
                    ...
                    %Normalize for firing rates.
                    n1size=length(selectcol);%*length(selectrow); %Add this tail back to do product-normalization
                    n2size=length(selectrow);
% %                     swap with below lines to remove spike-count normalization
% %                     n1size=1;%length(selectcol);%*length(selectrow);
% %                     n2size=1;%length(selectrow);
                    
%                     XMatrix1(trial_count+(shuff_count-1)*spikes.reps,:,pair_count)=cross/spikeproduct;
%                     % ^ = Collapsed across trials; reps still separate.
%                     NOTE: This collapse did not seem to greatly impact
%                     performance.  DJT 7/14/2013.  Commented out 8/29/2013

                    XMatrix1(trial_count+(shuff_count-1)*trial_total*spikes.reps,:,pair_count)=cross/n1size;
                    XMatrix2(trial_count+(shuff_count-1)*trial_total*spikes.reps,:,pair_count)=cross/n2size;
%                     % ^ =TRIALS SEPERATE.  Commented out 7/14/2013 DJT Brought back 8/29/2013                   
%                     XMatrix(trial_count,:,pair_count,shuff_count)=cross/spikeproduct;
                    trial_collapse1(pair_count,:)=trial_collapse1(pair_count,:)+cross/n1size;
                    trial_collapse2(pair_count,:)=trial_collapse2(pair_count,:)+cross/n2size;
                end;
...
fprintf (strcat('\nCompleted shuffle\f',num2str(num_iter),' out of\f',num2str(shuff_iterations),'.\f'))
toc
% if loop_timer>40
%  keyboard
% end
...
mean_matrix1=trial_collapse1/(trial_total*spikes.reps*shuff_iterations); 
mean_matrix2=trial_collapse2/(trial_total*spikes.reps*shuff_iterations); 
% % Activate below code to skip averaging and maintain raw spike counts
% % fprintf('\nNOTE! not dividing out number of trials or reps! Fix in raw and shuffle SpikeCorr_Anal\n')
% % mean_matrix1=trial_collapse1/shuff_iterations;%/(trial_total*spikes.reps*shuff_iterations); 
% % mean_matrix2=trial_collapse2/shuff_iterations;%/(trial_total*spikes.reps*shuff_iterations); 
...
shuffle_cell{1,1}=mean_matrix1;
shuffle_cell{1,2}=mean_matrix2;
shuffle_cell{2,1}=sem_matrix1;
shuffle_cell{2,2}=sem_matrix2;
shuffle_cell{3,1}=XMatrix1;
shuffle_cell{3,2}=XMatrix2;
