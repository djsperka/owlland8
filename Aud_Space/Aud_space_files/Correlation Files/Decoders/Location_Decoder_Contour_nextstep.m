...
%% Set controls
groups=[2]; %choose which conditions to look at
...
plot_cell_subplots=1; %when you make your contour plots, do you want to make subplots for each cell?
...
%     mean_cut_contour_scaled=mean_cut_contour_scaled ./ repmat(sum(sum(mean_cut_contour_scaled)),[nY,nX,1,1,1]);    
    ...
    mean_contour = squeeze ( mean(Zq_pop_mat,4)); %nY x nX x nC x nG
    mean_cut_contour=squeeze(mean(cut_contour,4)); %nY x nX x nC x nG
    mean_cut_contour_scaled=squeeze(mean(cut_contour_scaled,4)); %nY x nX x nC x nG
        
    if prod_or_mean==2
        %%% Add all the contour maps together and find their max values
        sum_contour=squeeze(sum(mean_cut_contour,3)); %nY x nX x nG
        combi_contour=sum_contour;
    elseif prod_or_mean==1
        %%% Calculate the product of the scaled contour maps
    ...
    [m,mean_xind]=max(max(combi_contour,[],1),[],2);
    [m,mean_yind]=max(max(combi_contour,[],2),[],1);
    ...
    for g=1:length(plot_groups)
        ...
        if plot_cell_subplots
            ...
            for i=1:nC;
                ...
                surf (Xq,Yq,mean_contour(:,:,i,plot_gr))
                xlabel('Azimuth')
                ylabel('Elevation')
                zlabel('Resp')
                alpha(0.5);
