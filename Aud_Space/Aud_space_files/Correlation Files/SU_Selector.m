function [pair_tagger,chan_tagger]=SU_Selector (unit_type, site_data, pair_tagger,chan_tagger)
...
elseif unit_type==2 %if only plotting single units
    ...
    chan_tagger(site_data.id_su~=1)=0;
...
elseif unit_type==3 %if only plotting multi units
    
    chan_tagger(site_data.id_su~=0)=0;
