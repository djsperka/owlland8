...
global rec snd;
if snd.interloom_type==1  %% added interloom condition 6/26/2013 DJT
...
else  %% Everything below this added 6/26/2013 DJT 
    if size(snd.data_arr1_pre,1)>1 && size(snd.data_arr1_pre,2)>1                  %%  display a contour plot if possible
        if snd.graph_type==1    %%display type is adjusted
            dispmat=(snd.data_arr1_post(:,:,rec.dispch)-((snd.posttime/snd.pre)*(snd.data_arr1_pre(:,:,rec.dispch))))/currentrep;   %%  subtract the average pre and devide by reps
        end
        if snd.graph_type==2    %%display type is pre
            dispmat=snd.data_arr1_pre(:,:,rec.dispch)/currentrep;   %%  subtract the average pre and devide by reps
        end
        if snd.graph_type==3    %%display type is post
            dispmat=snd.data_arr1_post(:,:,rec.dispch)/currentrep;   %%  subtract the average pre and devide by reps
        end
        if snd.graph_type==4    %%display type is post
            tempmax=max(max(snd.data_arr1_post(:,:,rec.dispch)));
            tempmin=min(min(snd.data_arr1_post(:,:,rec.dispch)));
            if tempmin==tempmax %%cannot be normalized
                dispmat=snd.data_arr1_post(:,:,rec.dispch);
            else
                dispmat=(snd.data_arr1_post(:,:,rec.dispch)-tempmin)*100/(tempmax-tempmin);
            end
        end
